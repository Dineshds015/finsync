...
export const checkBudgetAlert = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        for (const budget of budgets) {
            ...
            if (!defaultAccount) continue;   //Kkip if no default account
    ...
    async ({ event, step }) => {
        ...
        await step.run("process-transaction", async () => {
            ...
            await db.$transaction(async (tx) => {
                ...
                await tx.transaction.create(async (tx) => {
                    //Create new transaction
                    ...
                    await tx.account.update({
                        ...
                        data: { balance: { increament: balanceChange } },
                    });
...
export const generateMonthReports = inngest.createFunction({
...
async function generateFinancialInsights(stats, month) {
    ...
    const model = GeneratedCodeInfo.getGenerativeModel({ model: "gemini-1.5-flash" });
  ...
  `;
    ...
    try {
        ...
        return [
            ...
            "Track your recurring expenses to identity potential savings.",
        ];
    }
}
